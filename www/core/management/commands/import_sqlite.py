...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            raise CommandError(f"Não consegui abrir {path}: {exc}")

        conn.row_factory = sqlite3.Row

        counts = {}
        counts["agencias_penhor"] = self.import_agencias(conn)
        counts["licitacoes"] = self.import_licitacoes(conn)
        counts["clientes"], counts["telefones"] = self.import_clientes(conn)
        counts["contratos"] = self.import_contratos(conn)

        conn.close()

        # Carimba a data de atualização da base — base do freshness check
        # que impede o bot de repassar dados de contrato desatualizados.
    ...
    def import_licitacoes(self, conn):
    ...
    def import_clientes(self, conn):
        ...
        # Telefones: wipe & recreate per import run to keep it idempotent and simple.
        telefone_objs = []
    ...
    def import_contratos(self, conn):
        # Collect which contract numbers are flagged "parcelado" via the
        # client's contratos_parcelados list.
        parcelados = set()
